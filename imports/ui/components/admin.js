...
Template.adminPanel.onRendered(function () {
    this.autorun(() => {
    	if (!Meteor.user()?.device) {
    		this.subscribe('users', { 'profile.device': { $type: 'string' } });
...
Template.clientList.events({
    'click .button[id^=connect]'(e, template) {
        ...
        console.log(client?.connected || client?.reconnecting, client);

        /*if (client.reconnecting) $.toast({
			title: 'Connecting',
			message: `${ template.data[ 0 ].profile.name } connected.`,
			class : 'success',
			showIcon: 'check',
			showProgress: 'bottom'
		});*/
    ...
    'click .button[id^=disconnect]'(e, template) {
        ...
        const client = _.values(template.data[ 0 ].status.client)[ 0 ];
        console.log(client?.disconnected || client?.disconnecting, client);

        /*if (client.disconnecting) $.toast({
			title: 'Disconnecting',
			message: `${ template.data[ 0 ].profile.name } disconnected.`,
			class : 'error',
			showIcon: 'check',
			showProgress: 'bottom'
		});*/
